...
describe('captureMacOsWindow', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    mockExecaImpl.mockImplementation(async () => {
      return { stdout: '' };
    });
  });
  ...
  it('captures a window via osascript when iTerm is the app', async () => {
    mockExecaImpl.mockImplementation(async (bin: string) => {
      if (bin === 'osascript') {
        return { stdout: '12345\n' };
...
describe('MacOsSnapshotStrategy', () => {
  it('delegates capture() to captureMacOsWindow with the configured app name', async () => {
    ...
    expect(mockExecaImpl).toHaveBeenCalledWith('screencapture', expect.arrayContaining(['-l', '42', '/tmp/snap.png']));
  });
});
