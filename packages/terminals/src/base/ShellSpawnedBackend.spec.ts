...
type AppSpawner = import('@dragoscirjan/mcp-tuikit-spawn').AppSpawner;
...
type SpawnOptions = import('@dragoscirjan/mcp-tuikit-spawn').SpawnOptions;
...
describe('ShellSpawnedBackend', () => {
  let sessionHandler: import('vitest').Mocked<SessionHandler>;
  let snapshotStrategy: import('vitest').Mocked<SnapshotStrategy>;
  let spawner: import('vitest').Mocked<AppSpawner>;
  ...
  beforeEach(() => {
    ...
    } as unknown as import('vitest').Mocked<SessionHandler>;

    snapshotStrategy = {
      capture: vi.fn(),
    } as unknown as import('vitest').Mocked<SnapshotStrategy>;

    spawner = {
      spawn: vi.fn().mockResolvedValue({ pid: 1234, windowId: 'win-5678' }),
      kill: vi.fn().mockResolvedValue(undefined),
    } as unknown as import('vitest').Mocked<AppSpawner>;
  ...
  it('should spawn the app using the provided spawner', async () => {
    ...
    expect(spawner.spawn).toHaveBeenCalledWith({
      appName: 'DummyApp',
      executable: 'dummy',
      args: ['--tmux', expect.stringContaining('tmux'), '--session', 'test-session'],
    });
