...
- **Windows:** `winget install arndawg.tmux-windows` (Do _not_ use MSYS2 or WSL tmux if running natively).
...
- **No Native Headless Mode on Windows/macOS:** Spawning a native terminal (like Alacritty) on Mac/Windows _will_ open a physical window on your screen. True headless native rendering requires Linux (`Xvfb`/`Sway`/`kwin`). For invisible execution on Mac/Windows, you must use the `xterm.js` backend (via Playwright).
